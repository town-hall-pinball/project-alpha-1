...
class AttrCollection(object):
	...
	def __getattr__(self, attr):
		try:
			if type(attr) == str or type(attr) == unicode:
				if attr in self.__items_by_id:
					return self.__items_by_id[attr]
	...
	def add(self, item, value):
		...
		if hasattr(value, 'json_number'):
			self.__items_by_id[value.json_number] = value
	...
	def __contains__(self, item):
		if type(item) == str or type(item) == unicode:
			return self.__items_by_name.has_key(item) or self.__items_by_id.has_key(item)
		else:
...
class GameItem(object):
	...
	_number = None
	"""Number string from JSON config file"""
	tags = None
