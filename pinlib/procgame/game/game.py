...
class GameController(object):
	...
	config = None
	"""JSON game configuration loaded by :meth:`load_config`."""
	balls_per_game = 3
	...
	def load_config(self, filename):
		"""Reads the JSON machine configuration file into memory.
	...
	def load_config_stream(self, stream):
		"""Reads the JSON machine configuration in stream form (string or opened file) into memory.
		Configures the switches, lamps, and coils members.
		Enables notifyHost for the open and closed debounced states on each configured switch.
		"""
		self.config = json.load(stream)
		if not self.config:
			raise ValueError, 'load_config_stream() could not load configuration.  Malformed JSON?'
		print self.config
		self.process_config()
	...
	def process_config(self):
		...
		# the JSON settings to see how many PDBs are being used.  Then we can configure
		# the P-ROC appropriately to use those PDBs.  Only then can we relate the JSON
		...
		for section, collection, klass in pairs:
			if section in self.config:
				...
				for name in sect_dict:
					...
					# Find the P-ROC number for each item in the JSON sections.  For PDB's
					...
					if self.machine_type == pinproc.MachineTypePDB:
						number = pdb_config.get_proc_number(section, str(item_dict['id']))
						if number == -1:
							self.logger.error('%s Item: %s cannot be controlled by the P-ROC.  Ignoring...', section, name)
							continue
					else:
						number = pinproc.decode(self.machine_type, str(item_dict['id']))
		...
		#	print led.name
		#	print led.json_number


	def load_settings(self, template_filename, user_filename):
		"""Loads the JSON game settings configuration file.  The game settings
		...
		*user_filename* contains the values set by the user.
		...
		"""
		self.user_settings = {}
		self.settings = json.load(open(template_filename, 'r'))
		if os.path.exists(user_filename):
			self.user_settings = json.load(open(user_filename, 'r'))
	...
	def save_settings(self, filename=None):
		...
		json.dump(self.user_settings, stream)

	def load_game_data(self, template_filename, user_filename):
		"""Loads the JSON game data configuration file.  This file contains
		...
		*user_filename* contains the values set by the user.
		...
		"""
		self.game_data = {}
		template = json.load(open(template_filename, 'r'))
		if os.path.exists(user_filename):
			self.game_data = json.load(open(user_filename, 'r'))
	...
	def save_game_data(self, filename):
		...
		if os.path.exists(filename):
			os.remove(filename)
		stream = file(filename, 'w')
		json.dump(self.game_data, stream)
