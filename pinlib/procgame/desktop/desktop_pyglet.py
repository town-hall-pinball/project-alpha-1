...
class FrameDrawer(object):
	...
	def draw(self, frame):
		...
		if not self.image:
			self.image = pyglet.image.ImageData(DMD_SIZE[0], DMD_SIZE[1], 'L', data, pitch=DMD_SIZE[0])
		else:
			self.image.set_data('L', DMD_SIZE[0], data)
		gl.glTexParameteri(self.image.get_texture().target, gl.GL_TEXTURE_MAG_FILTER, gl.GL_NEAREST)
		self.image.blit(0, 0, width=DMD_SIZE[0]*DMD_SCALE, height=DMD_SIZE[1]*DMD_SCALE)
